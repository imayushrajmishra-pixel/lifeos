...
import {
  ...
  GraduationCap, FolderKanban, ListChecks, Award, Library, Film, Bookmark, BarChart3, Settings, LogOut, MessageSquare,
} from 'lucide-react';
...
const groups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/private/movies', label: 'Movies', icon: Film },
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-paper md:hidden">
          ...
          <div className="mt-8 flex items-center justify-between px-7">
            <ThemeToggle />
            <form action={signOut}>
              <button className="flex items-center gap-1.5 text-xs text-muted hover:text-ink focus-ring">
                <LogOut size={13} /> Sign out
