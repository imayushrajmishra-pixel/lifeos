...
export default async function SettingsPage({ searchParams }: { searchParams: { updated?: string; error?: string } }) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-14">
      ...
      <section>
        <h2 className="font-display text-xl text-ink">Music</h2>
        <p className="mt-1 text-sm text-muted">Manual for now — wire up Spotify in lib/actions/profile.ts once SPOTIFY_CLIENT_ID is set.</p>
        <Card className="mt-4">
          <form action={updateProfile} className="space-y-4">
            ...
            <input type="hidden" name="what_i_like" value={profile?.what_i_like} />
            <input type="hidden" name="philosophy" value={profile?.philosophy} />
            <input type="hidden" name="closing_line" value={profile?.closing_line} />
            <input type="hidden" name="social_links" value={JSON.stringify(profile?.social_links || [])} />
            <div><Label htmlFor="now_playing">Current song</Label><Input id="now_playing" name="now_playing" defaultValue={profile?.now_playing ?? ''} /></div>
            <div><Label htmlFor="favorite_artists">Favorite artists (comma separated)</Label><Input id="favorite_artists" name="favorite_artists" defaultValue={profile?.favorite_artists?.join(', ')} /></div>
            <div><Label htmlFor="favorite_songs">Favorite songs (comma separated)</Label><Input id="favorite_songs" name="favorite_songs" defaultValue={profile?.favorite_songs?.join(', ')} /></div>
